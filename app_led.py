...
def generar_texto_reporte(req_w, req_h, res_hw, calc_proc, calc_pwr, calc_rig):
    ...
    texto += "\n" + "-" * 50 + "\n"
    texto += "[E] CONSUMO ELECTRICO Y AA (Opcion 1)\n"
    for k, v in calc_pwr.calcular_energia_y_clima().items():
        texto += f"  > {k}: {v}\n"

    texto += "\n" + "-" * 50 + "\n"
    texto += "[F] INGENIERIA ESTRUCTURAL Y ELEVACION (Opcion 1)\n"
...
with st.sidebar:
    ...
    dist_cable = st.number_input("Distancia a procesador (m)", min_value=1.0, value=50.0, step=10.0, format="%.1f")
